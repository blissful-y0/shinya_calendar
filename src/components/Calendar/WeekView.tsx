...
import { getDaysInWeek, isCurrentDay, isSameDayAs } from "@utils/calendar";
...
const WeekView: React.FC = () => {
  ...
  const getEventsForDateLocal = (date: Date) => {
    ...
    return getEventsForDate(events, date, rangeStart, rangeEnd).sort((a, b) => {
  ...
  const getEventPosition = (
    ...
    if (event.endDate && event.date !== event.endDate) {
      ...
      // 이벤트가 여러 날에 걸쳐있는 경우
      const eventStartDay = weekDays.findIndex((day) =>
        isSameDayAs(day, eventStart)
      );
      const eventEndDay = weekDays.findIndex((day) =>
        isSameDayAs(day, eventEnd)
      );

      if (
        eventStartDay !== -1 &&
        eventEndDay !== -1 &&
        eventStartDay <= dayIndex &&
        dayIndex <= eventEndDay
      ) {
        // 이벤트가 현재 주 내에서 여러 날에 걸침
        if (dayIndex === eventStartDay) {
          // 시작 날짜
          const daysSpanning = Math.min(
            eventEndDay - eventStartDay + 1,
            7 - eventStartDay
          );
                  ...
                  return (
                    <div
                      ...
                      className={`${styles.timedEvent} ${
                        position.isMultiDay ? styles.multiDay : ""
                      }`}
                      style={{
                        top: `${position.top}px`,

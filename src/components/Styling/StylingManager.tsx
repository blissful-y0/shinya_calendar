...
const StylingManager: React.FC<StylingManagerProps> = ({ onClose }) => {
  ...
  const [stickerEditMode, setStickerEditMode] = useRecoilState(stickerEditModeState);
  ...
  const handleClearAllStickers = () => {
    if (confirm('모든 스티커를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
      setStickers([]);
    }
  };
    ...
    if (confirm(confirmMessage)) {
      try {
        ...
        if (window.electronAPI && window.electronAPI.resizeWindow) {
          await window.electronAPI.resizeWindow(layout.resolution.width, layout.resolution.height);
        }
    ...
    <div className={styles.overlay} onClick={handleBackdropClick}>
      <div className={styles.panel}>
        <div className={styles.header}>
          <h2 className={styles.title}>스타일링 매니저</h2>
        ...
        <div className={styles.tabs}>
          <button
            className={`${styles.tab} ${activeMode === 'sticker' ? styles.active : ''}`}
          ...
          <button
            className={`${styles.tab} ${activeMode === 'theme' ? styles.active : ''}`}
              ...
              <div className={styles.infoSection}>
                <div className={styles.canvasInfo}>
                  <div className={styles.canvasHeader}>
                    <h3>캔버스 관리</h3>
                    <button
                      className={`${styles.editModeToggle} ${stickerEditMode ? styles.active : ''}`}
                      onClick={toggleStickerEditMode}
                      title={stickerEditMode ? '편집 모드 끄기' : '편집 모드 켜기'}
                    >
                  ...
                  {!stickerEditMode && (
                    <p className={styles.hint}>편집 시작을 누르면 하단에 툴바가 나타납니다.</p>
                  )}
                        ...
                        <div className={styles.layoutActions}>
                          ...
                          <div className={styles.layoutMenu}>
                            <button
                              ...
                              onClick={() => setLayoutMenuId(layoutMenuId === layout.id ? null : layout.id)}
                              title="더보기"
                            >
                              <MdMoreVert />
                            </button>

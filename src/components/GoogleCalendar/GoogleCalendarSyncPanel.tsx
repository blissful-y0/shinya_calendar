...
export const GoogleCalendarSyncPanel: React.FC<GoogleCalendarSyncPanelProps> = ({ onClose }) => {
  const syncState = useRecoilValue(googleCalendarSyncState);
  const events = useRecoilValue(eventsState);
  const { importFromGoogle, exportMultipleToGoogle, syncBidirectional, isSyncing } =
        ...
        <div className={styles.content}>
          ...
          <div className={styles.body}>
            <p className={styles.notConnected}>
              구글 캘린더에 연동되어 있지 않습니다. 먼저 연동 설정을 완료해주세요.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className={styles.body}>
          ...
          {syncState.autoSync && (
            <div className={styles.autoSyncNotice}>
              <p>
                ℹ️ 자동 동기화가 활성화되어 있습니다. 이벤트를 추가/수정하면 자동으로 구글
                캘린더와 동기화됩니다.
              </p>
            </div>
          )}
